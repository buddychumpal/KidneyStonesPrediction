...
To narrow our choices down, I will remove all columns where more than 50% of the  
data was NAs.
We also know while the medical conditions were highly associated with kidney stones,  
they are also highly associated with each other, so we could reduce dimensionality  
by either removing some or using feature combinations.

```{r}
df_subset <- df[, which(colMeans(!is.na(df)) > 0.5)]
```
