---
title: "Predicting Occurance of Kidney Stones Using NHANES 2017 - 2020 Data"
author: |
...
df$DBD895[df$DBD895==9999 | df$DBD895==5555 | df$DBD895==7777] <- NA
```
...
```{r}
set.seed(5)
df$KIQ026 <- as.factor(df$KIQ026)
train <- df %>% dplyr::slice_sample(prop = 0.75)
test  <- dplyr::anti_join(df, train, by = 'SEQN')
x_train <- subset(train, select = -c(KIQ026))
y_train <- subset(train, select = c(KIQ026))
x_test <- subset(test, select = -c(KIQ026))
y_test <- subset(test, select = c(KIQ026))
```

I started with 145 predictors to choose from, which is too many for a model. 
I decided to first make a small model, with predictors that didn't have any NA values,
and appeared to have a link with kidney stones in our EDA (demographics  
and medical conditions). From there I can see the effectiveness of these predictors alone,  
and the effectiveness of a randomForest model when our target is a minority.

My initial thinking was a decision tree classifier, so patients would be able to  
clearly understand why diagnostic decisions were made, but I decided on a  
randomForest model as it allows for more complexity than a singular decision tree.

```{r, results = 'hide'}
factors <- c("RIAGENDR", "RIDAGEYR", "RIDRETH3", "DMDMARTZ","BPQ020", "BPQ080", "DIQ010")
train_subset <- subset(train, select = c(RIAGENDR,RIDAGEYR, RIDRETH3,
																				 DMDMARTZ,BPQ020, BPQ080, DIQ010,KIQ026))
train_subset %>% mutate_at(factors, factor)
```

Outcome of initial, small, randomForest model:

```{r}
first_for <- randomForest(KIQ026 ~., data = train_subset)
print(first_for)
```
This model classed everything as the majority (no kidney stones), likely due to  
the high prevalence of the majority. I decided to combat this by using ROSE
(Randomly Over Sampling Examples) so the minority would be more prevalent.

```{r}
data.rose <- ROSE(KIQ026~., data = train_subset, seed = 5, p = 0.45)$data
```

The same randomForest model after applying ROSE:

```{r}
first_for_rose <- randomForest(KIQ026 ~., data = data.rose)
print(first_for_rose)
```

With the majority no longer overwhelming, we can see this model is okay, but not  
ideal, with a 27% error rate. More worryingly, the error rate for 

To narrow our choices down, I will remove all columns and rows where more than 50% of the  
data was NAs, then further investigate the data.
For imputation, due to the large amount of data, kNN takes too much time, so I will  
be using mean and mode instead.

```{r}
df_subset <- df[which(rowMeans(!is.na(df)) > 0.5), which(colMeans(!is.na(df)) > 0.5)]
```


```{r, results='hide'}

factors <- c("RIAGENDR", "RIDRETH3", "DMDEDUC2", "DMDMARTZ", "DBQ095Z", "DBD100", "DRQSPREP",
	"DR1TWSZ", "DSDANTA", "BPQ020", "BPQ080", "DIQ010", "DIQ160", "DBQ700",
	"KIQ022", "KIQ026", "KIQ005","KIQ042", "KIQ044", "KIQ046", "PAQ605",
	"PAQ620", "PAQ635", "PAQ650", "PAQ665", "SMQ020", "SMD460","SLQ050") 
df_subset %>% mutate_at(factors, factor)
subset_train <- df_subset %>% dplyr::slice_sample(prop = 0.75)
subset_test  <- dplyr::anti_join(df_subset, subset_train, by = 'SEQN')
```

```{r}
missing_num <- c("INDFMPIR", "BMXBMI", "BPXOSY1", "BPXOSY2", "BPXOSY3",
								 "BPXODI1", "BPXODI2", "BPXODI3", "BPXOPLS1", "BPXOPLS2",
								 "BPXOPLS3", "LBXSBU", "LBXSCA", "LBXSUA", "URDACT", "URDFLOW1",
								 "DBD900", "DBD905", "DBD910", "KIQ480", "PAD680","ALQ130",
								 "RXDDAYS", "RXDCOUNT")

train_subset_missn <- subset(subset_train, select = missing_num)
test_subset_missn <- subset(subset_test, select = missing_num)
mean_col <- colMeans(train_subset_missn, na.rm = TRUE)

miss_cat <- c("DBD100","DSDANTA","DIQ160","KIQ005","KIQ042","KIQ044","KIQ046",
							"SMD460")
train_subset_missc <- subset(subset_train, select = miss_cat)
test_subset_missc <-subset(subset_test, select = miss_cat)

for(i in colnames(train_subset_missn)){
  train_subset_missn[,i][is.na(train_subset_missn[,i])] <- mean_col[i]
  test_subset_missn[,i][is.na(test_subset_missn[,i])] <- mean_col[i]}

train_subset_missn <- scale(train_subset_missn)
test_subset_missn <- scale(test_subset_missn)

for (i in 1:ncol(train_subset_missc)){
  mod_val <- as.factor(as.numeric(Mode(train_subset_missc[,i], na.rm=TRUE)))
  train_subset_missc[,i][is.na(train_subset_missc[,i])] <- mod_val
  test_subset_missc[,i][is.na(test_subset_missc[,i])] <- mod_val}

for (i in colnames(train_subset_missc)){
	subset_train[,i] <- train_subset_missc[,i]
	subset_test[,i] <- test_subset_missc[,i]
}

for (i in colnames(train_subset_missn)){
	subset_train[,i] <- train_subset_missn[,i]
	subset_test[,i] <- test_subset_missn[,i]
}

subset.rose <- ROSE(KIQ026~., data = subset_train, seed = 5, p = 0.35)$data
table(subset.rose$KIQ026)
```
