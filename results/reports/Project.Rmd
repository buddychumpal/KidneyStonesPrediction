...
146 predictors is too many for a model, I want to first try a small model based  
off values that appeared to have a link with kidney stones in our EDA (demographics  
and medical conditions), and see that effectiveness, then fine tune.  

My initial thinking was a decision tree classifier, so patients would be able to  
clearly understand why diagnostic decisions were made. For more complexity and  
higher accuracy however, I have instead gone with a random forest model.
...
print(first_for)
```
This is not an effective model, as it is just classifying everything as the majority,  
so everyone is classified as no kidney stones, even when they have it. This is  
because of the imbalance in our data. As well as testing out other predictors,  
I will use ROSE so there are more instances of the minority (kidney stones being  
present), which will also hopefully improve prediction abilities.

```{r}
data.rose <- ROSE(KIQ026~., data = train_subset, seed = 5, p = 0.35)$data
table(data.rose$KIQ026)
```
```{r}
...
While the model is now predicting outcomes other than 2, it actually has a much  
higher error rate. So I definitely want to investigate other predictors I can use.  
...
df_subset_missn <- subset(df_subset, select = missing_num)
mean_col <- colMeans(df_subset_missn, na.rm = TRUE)

miss_cat <- c("DBD100","DSDANTA","DIQ160","KIQ005","KIQ042","KIQ044","KIQ046",
							"SMD460")
df_subset_missc <- subset(df_subset, select = miss_cat)

for(i in colnames(df_subset_missn)){
  df_subset_missn[,i][is.na(df_subset_missn[,i])] <- mean_col[i]}

df_subset_missn <- scale(df_subset_missn)

for (i in 1:ncol(df_subset_missc)){
  mod_val <- as.factor(as.numeric(Mode(df_subset_missc[,i], na.rm=TRUE)))
  df_subset_missc[,i][is.na(df_subset_missc[,i])] <- mod_val}

for (i in colnames(df_subset_missc)){
	df_subset[,i] <- df_subset_missc[,i]
}

for (i in colnames(df_subset_missn)){
	df_subset[,i] <- df_subset_missn[,i]
}

subset.rose <- ROSE(KIQ026~., data = df_subset, seed = 5, p = 0.35)$data
